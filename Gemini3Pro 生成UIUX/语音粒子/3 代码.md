...
<head>
    ...
    <title>AI Voice Interface - Slow Scatter</title>
    <style>
        :root {
            --gold-color: #b48c50;
            --bg-color: #ffffff;
    ...
    <script>
        ...
        function animate() {
            ...
            particles.forEach(p => {
                ...
                if (feedback > 0.05) {
                    // --- 速度调整核心 ---
                    // 之前的倍率是 2.5，现在减半为 1.25，让粒子飞得更慢
                    const scatterStrength = feedback * 1.25; 
                    p.vx += (Math.random() - 0.5) * scatterStrength;
                    p.vy += (Math.random() - 0.5) * scatterStrength;
                    p.vz += (Math.random() - 0.5) * scatterStrength;
                    
                    // 向外的推力也减半 (0.5 -> 0.25)
                    p.vx += (p.baseX / baseRadius) * feedback * 0.25;
                    p.vy += (p.baseY / baseRadius) * feedback * 0.25;
                }
