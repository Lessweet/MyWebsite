...
final class HomeViewModel {
    ...
    func startDemoScan() async {
        ...
        // 创建演示数据
        let demoGroup = createDemoPhotoGroup()
        photoGroups = [demoGroup]
        totalSpaceSavable = demoGroup.estimatedSpaceSaved
        totalPhotosScanned = demoGroup.photos.count
    ...
    /// 创建演示用的照片组
    private func createDemoPhotoGroup() -> PhotoGroup {
        // 使用 DemoPhotoFactory 创建演示照片
        let demoPhotos = DemoPhotoFactory.createDemoPhotos(count: 6)

        let group = PhotoGroup(photos: demoPhotos, clusterType: .visualSimilarity)

        // 设置第一张为最佳照片
        group.setBestPhoto(at: 0)
