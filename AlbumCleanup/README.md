...
- **液态玻璃 UI**：iOS 26 Liquid Glass 设计语言，液态透镜材质。通过使用 SwiftUI、UIKit 和 AppKit 的标准组件，利用这些材料，使用最少的代码。
...
│                                                                 │
│   适用场景：                                                     │
│   - 扫描结果页 Quick Clean 按钮                                  │
│   - 模态页 LiquidGlassActionBar                                 │
│                                                                 │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│   【底部渐变蒙层规范】                                            │
...
│       LinearGradient(                                           │
│           stops: [                                              │
│               .init(color: Color(uiColor: .systemBackground)    │
...
## 卡片展开交互原型 (Card Expansion Prototype)

> App Store Today 风格卡片展开 → 模态页
...
│     ├── 卡片照片 ID: "photo-\(group.id)"                         │
│     └── 模态页首图使用相同 ID，实现无缝过渡                        │
│                                                                 │
│  2. fullScreenCover / sheet                                     │
│     ├── 使用 .fullScreenCover 替代 navigationDestination        │
│     └── 配合 presentationBackground(.clear) 实现透明背景          │
│                                                                 │
│  3. 展开动画时序                                                 │
│     ├── 0ms:   照片开始放大                                      │
│     ├── 100ms: 背景开始模糊/变暗                                  │
...
│                                                                 │
│  4. 下拉关闭手势                                                 │
│     ├── iOS 16+ interactiveDismissDisabled(false)               │
│     └── 配合 DragGesture 实现跟手缩放效果                         │
│                                                                 │
│  5. 状态同步                                                     │
│     ├── 卡片封面 = group.bestPhoto (AI最佳)                      │
│     └── 模态页首图 = 同一张 bestPhoto                            │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```
...
```
     App Store Today 卡片                    PureShot 照片合集卡片
    ┌─────────────────────┐                ┌─────────────────────┐
    │                     │                │                     │
    │    [App 封面图]      │                │   [AI 最佳照片]      │
    │                     │                │                     │
...
| 阶段 | 动作 | 视觉效果 | 动画参数 |
|------|------|----------|----------|
| 点击卡片 | tap | 卡片照片开始放大 | spring(0.4, 0.85) |
| 展开中 | - | 背景模糊变暗，圆角渐变 | 100ms delay |
| 展开完成 | - | 模态页完全显示 | 400ms 总时长 |
| 下拉关闭 | drag | 视图缩小+圆角增加 | 跟手动画 |
| 关闭完成 | release | 照片回归卡片位置 | matchedGeometry |
